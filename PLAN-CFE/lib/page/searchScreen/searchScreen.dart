...
class _SearchScreenState extends State<SearchScreen> {
  ...
  Future<void> fetchData() async {
    ...
    final circuitosResponse = await Dio().get(
      'https://apicfe.onrender.com/api/numcircs',
      options: Options(
        headers: {
          'Authorization': 'Bearer $token'
        }, // Agregar el token en los headers
  ...
  List<dynamic> performSearch(String query, SearchType searchType) {
    ...
    if (searchType == SearchType.area) {
      ...
      print("Total circuitos: ${_circuitos.length}"); // Agrega esta línea
      results.addAll(_circuitos.where(
        (circuito) =>
            circuito.circuito != null &&
            circuito.circuito!.nombre != null &&
          ...
          Expanded(
            child: ListView.builder(
              ...
              itemBuilder: (context, index) {
                ...
                return ListTile(
                  title: result is Circuito
                      ? Text(result.circuito!.nombre)
                      : Text(result.nombre),
                  onTap: () {
